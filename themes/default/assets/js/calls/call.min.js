...
//conn = new WebSocket("wss://34.73.87.227:8089/ws", "sip");
//conn = new WebSocket("wss://"+txtRealm.value+":8089/ws", "sip");
//window.onbeforeunload = function() {
//    var e = true;
//    if(e){
//        toastr['error']('Mute / Unmute failed__');
//    }else{
//        toastr['error']('Mute / Unmute failed');
//    }
//    
...
var txtCallStatusClass = $('.txtCallStatus');

C = { divKeyPadWidth: 220 };

window.onload = function () {
    $(".active__call").css("opacity",0);
...
//        
//            conn = new WebSocket("wss://pbx.collectpay.co:8089/ws", "sip");
//            conn.readyState();
//            
//            var txtWebsocketServerUrl = "wss://pbx.collectpay.co:8089/ws";
//
...
//            txtPhoneNumber.value = s_value;
        ...
        txtCallStatus.innerHTML = 'Video ' + (bDisableVideo ? 'disabled' : 'enabled');        
    }
}
...
//            btnRegister.disabled = false;
    ...
    txtRegStatus.innerHTML = '<b>Login</b>';
//    btnRegister.disabled = false;
}
...
//        btnCall.disabled = true;
        ...
        if (oSipSessionCall.call(txtPhoneNumber.value) != 0) {
            ...
            txtCallStatusClass.html('Failed to make call');
//            btnCall.disabled = false;
        ...
        txtCallStatusClass.html('Connecting...');
...
function sipShareScreen() {
    if (SIPml.getWebRtcType() === 'w4a') {
        ...
        if (!oSipSessionCall) {
            txtCallStatus.innerHTML = 'No active session';
            txtCallStatusClass.html('No active session');
            return;
        }
        if (oSipSessionCall.bfcpSharing) {
            if (oSipSessionCall.stopBfcpShare(oConfigCall) != 0) {
                ...
                txtCallStatusClass.html('Failed to stop BFCP share');
            } else {
            ...
            if (oSipSessionCall.startBfcpShare(oConfigCall) != 0) {
                ...
                txtCallStatusClass.html('Failed to start BFCP share');
            } else {
...
//            btnTransfer.disabled = true;
            if (oSipSessionCall.transfer(s_destination) != 0) {
                txtCallStatus.innerHTML = 'Call transfer failed';
                txtCallStatusClass.html('Call transfer failed');
//                btnTransfer.disabled = false;
                return;
            }
            txtCallStatus.innerHTML = 'Transfering the call...';
            txtCallStatusClass.html('Transfering the call...');
        }
    }
}
...
function sipToggleHoldResume() {
    if (oSipSessionCall) {
        ...
        txtCallStatus.innerHTML = oSipSessionCall.bHeld ? 'Resuming the call...' : 'Holding the call...';
        txtCallStatusClass.html(oSipSessionCall.bHeld ? 'Resuming the call...' : 'Holding the call...');
        // hold fas fa-pause
        // resume fas fa-play
        i_ret = oSipSessionCall.bHeld ? oSipSessionCall.resume() : oSipSessionCall.hold();
        if (i_ret != 0) {
            txtCallStatus.innerHTML = 'Hold / Resume failed';
            txtCallStatusClass.html('Hold / Resume failed');
...
function sipToggleMute() {
    ...
    if (oSipSessionCall) {
        ...
        txtCallStatus.innerHTML = bMute ? 'Mute the call...' : 'Unmute the call...';
        txtCallStatusClass.html(bMute ? 'Mute the call...' : 'Unmute the call...');
        i_ret = oSipSessionCall.mute('audio'/*could be 'video'*/, bMute);
        if (i_ret != 0) {
            toastr['error']('Mute / Unmute failed');
            return;
        }
        // mute fas fa-microphone-alt
        // unmute fas fa-microphone-alt-slash
...
function sipHangUp() {
    if (oSipSessionCall) {
        ...
        txtCallStatusClass.html('Terminating the call...');
...
function onKeyUp(evt) {
    ...
    if (evt.keyCode == 27) {
        ...
        if (evt.keyCode == 65 || evt.keyCode == 86) { // A (65) or V (86)
            ...
            txtCallStatus.innerHTML = 'Video ' + (bDisableVideo ? 'disabled' : 'enabled');
            txtCallStatusClass.html((bDisableVideo ? 'disabled' : 'enabled'));
...
function getInfoCall(){
    var callerId = txtDisplayName.value;
    var number = txtPhoneNumber.value;
    $.ajax({
            url:  SITEURL + "/general/getInfoCall",
            async: false,
            dataType: 'json',
            type: 'POST',
            data : {callerid : callerId, number : number},
            success: function (result) {                
                if(result.status == 'success'){
                    var callJson = '{"id":"'+result.model.id+'","state":"'+result.model.estado+'","ext":"'+result.model.extension+'","date":"'+result.model.fecha+'","number":"'+result.model.numero+'","record":"'+result.model.record+'","uniqueid":"'+result.model.uniqueid+'"}'
                    $('#is_contact').val((result.model.estado == 'ANSWER')? 1 : 0);               
                    $('#callInfo').val(callJson);
//                    $('#ta-comment').val(callJson);
                }else{
                    console.log('error');
                }
            }
...
function uiCallTerminated(s_description,terminated) {
    
    if(terminated){               
        getInfoCall();
    }
    
    uiBtnCallSetText("Llamar");
    //btnHangUp.value = 'Colgar';
    //btnHoldResume.value = 'Retener';
//    btnMute.value = "Silenciar";
//    btnCall.disabled = false;
    btnHangUp.disabled = true;
    if (window.btnBFCP)
    ...
    txtCallStatus.innerHTML = s_description;
    txtCallStatusClass.html(s_description);
    ...
    setTimeout(function () {
        if (!oSipSessionCall){
            $('#management-submit').removeClass('hide');
            $('#management-back').addClass('hide');
            txtCallStatus.innerHTML = '';
            txtCallStatusClass.html('LLAMADA FINALIZADA');
            $('#content-form-management').append($('#content-form-call').detach());
            $('.phone__content__inner').hide();
...
//                btnCall.disabled = false;
                ...
                txtCallStatus.innerHTML = "Incoming call from [<b>" + sRemoteNumber + "</b>]";
                txtCallStatusClass.html('Incoming call from [<b>'+ sRemoteNumber + '</b>]');
...
//            divGlassPanel.style.visibility = 'hidden';
            if (e.type == 'm_permission_refused') {
                uiCallTerminated('Media stream permission denied',false);
            }
            break;
        }
...
//                btnTransfer.disabled = false;
                ...
                txtCallStatusClass.html(e.description);
                divCallOptions.style.opacity = bConnected ? 1 : 0;
                $(".active__call").css("opacity", bConnected ? 1 : 0);
        ...
        case 'terminated':
        {
            if (e.session == oSipSessionRegister) {
                ...
                uiCallTerminated(e.description,true);
        ...
        case 'i_ao_request':
        {
            if (e.session == oSipSessionCall) {
                ...
                if (iSipResponseCode == 180 || iSipResponseCode == 183) {
                    startRingbackTone();
                    txtCallStatus.innerHTML = 'Remote ringing...';
                    txtCallStatusClass.html('Remote ringing...');
                }
            }
            break;
        }
        ...
        case 'm_early_media':
        {
            if (e.session == oSipSessionCall) {
                stopRingbackTone();
                stopRingTone();
                txtCallStatus.innerHTML = 'Early media started';
                txtCallStatusClass.html('Early media started');
            }
            break;
        }
        ...
        case 'm_local_hold_ok':
        {
            if (e.session == oSipSessionCall) {
                ...
                //btnHoldResume.value = 'Continuar';
                //btnHoldResume.disabled = false;
                txtCallStatus.innerHTML = 'Call placed on hold';
                txtCallStatusClass.html('Call placed on hold');
        ...
        case 'm_local_hold_nok':
        {
            if (e.session == oSipSessionCall) {
                ...
                //btnHoldResume.value = 'Retener';
                //btnHoldResume.disabled = false;
                txtCallStatus.innerHTML = 'Failed to place remote party on hold';
                txtCallStatusClass.html('Failed to place remote party on hold');
            }
            break;
        }
        case 'm_local_resume_ok':
        {
            if (e.session == oSipSessionCall) {
                ...
                //btnHoldResume.value = 'Retener';
                //btnHoldResume.disabled = false;
                txtCallStatus.innerHTML = 'Call taken off hold';
                txtCallStatusClass.html('Call taken off hold');
        ...
        case 'm_local_resume_nok':
        {
            if (e.session == oSipSessionCall) {
                oSipSessionCall.bTransfering = false;
                //btnHoldResume.disabled = false;
                txtCallStatus.innerHTML = 'Failed to unhold call';
                txtCallStatusClass.html('Failed to unhold call');
            }
            break;
        }
        case 'm_remote_hold':
        {
            if (e.session == oSipSessionCall) {
                txtCallStatus.innerHTML = 'Placed on hold by remote party';
                txtCallStatusClass.html('Placed on hold by remote party');
            }
            break;
        }
        ...
        {
            if (e.session == oSipSessionCall) {
                txtCallStatus.innerHTML = 'Taken off hold by remote party';
                txtCallStatusClass.html('Taken off hold by remote party');
            }
            break;
        }
        case 'm_bfcp_info':
        {
            if (e.session == oSipSessionCall) {
                txtCallStatus.innerHTML = 'BFCP Info: ' + e.description;
                txtCallStatusClass.html('BFCP Info: ' + e.description);
            }
            break;
        }
        ...
        {
            if (e.session == oSipSessionCall) {
                txtCallStatus.innerHTML = 'Call transfer in progress...';
                txtCallStatusClass.html('Call transfer in progress...');
            }
            break;
        }
        case 'o_ect_accepted':
        {
            if (e.session == oSipSessionCall) {
                txtCallStatus.innerHTML = 'Call transfer accepted';
                txtCallStatusClass.html('Call transfer accepted');
            }
            break;
        }
        ...
        {
            if (e.session == oSipSessionCall) {
                txtCallStatus.innerHTML = 'Call transfer completed';
                txtCallStatusClass.html('Call transfer completed');
//                btnTransfer.disabled = false;
...
//                btnTransfer.disabled = false;
        ...
        {
            if (e.session == oSipSessionCall) {
                txtCallStatus.innerHTML = "Call Transfer: <b>" + e.getSipResponseCode() + " " + e.description + "</b>";
                txtCallStatusClass.html('Call Transfer: <b>' + e.getSipResponseCode() + ' ' + e.description + '</b>');
